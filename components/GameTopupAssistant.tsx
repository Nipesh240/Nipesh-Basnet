...
const GameTopupAssistant: React.FC<GameTopupAssistantProps> = ({ onClose, addActivity }) => {
  const [step, setStep] = useState<1 | 2 | 3>(1); // 1: Select Game, 2: Details, 3: Payment
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size / 1024 > MAX_FILE_SIZE_KB) {
        setError(`File size exceeds 300KB protocol limit.`);
        return;
      }
      setVoucher({
        name: file.name,
  ...
  if (isSuccess) {
    return (
      <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-md bg-[#0a0f1d] border border-purple-500/20 rounded-[3rem] p-12 text-center animate-in zoom-in duration-300">
          <div className="w-20 h-20 bg-purple-500/20 text-purple-500 rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 shadow-[0_0_40px_rgba(168,85,247,0.3)]">
            <CheckCircle2 className="w-10 h-10" />
          </div>
          <h3 className="text-3xl font-black uppercase tracking-tight text-white mb-4">Uplink Confirmed</h3>
          <p className="text-slate-500 text-[11px] uppercase tracking-[0.2em] mb-10 leading-relaxed">
            Your topup for <span className="text-white font-black">{selectedGame?.name}</span> is in the domestic queue. Delivery typical in 5-15 mins.
          </p>
          <button onClick={onClose} className="w-full bg-white text-black py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.5em] active:scale-95 transition-all">Close Node</button>
        </div>
      </div>
    );
  }
      ...
      <div className="relative w-full max-w-2xl bg-[#0a0f1d] border border-white/10 rounded-[3.5rem] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-300">
        ...
        <div className="p-8 border-b border-white/5 bg-white/2 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-purple-500/10 flex items-center justify-center text-purple-400 border border-white/5 shadow-inner">
              <Gamepad2 className="w-6 h-6" />
            </div>
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-purple-500 uppercase tracking-[0.4em]">Step {step} of 3</span>
              <h3 className="text-xl font-black text-white uppercase tracking-tight">Game Topup</h3>
            </div>
          </div>
        ...
        <div className="p-10 max-h-[55vh] overflow-y-auto custom-scrollbar">
                ...
                {GAME_LIST.map(game => (
                  <button 
                    ...
                    className={`flex items-center justify-between p-6 rounded-3xl transition-all group shadow-lg border ${
                       selectedGame?.id === game.id ? 'border-purple-500/40 bg-purple-500/5' : 'bg-white/2 border-white/5 hover:border-purple-500/20'
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-slate-500 group-hover:text-purple-400 transition-colors">
              ...
              <div className="p-8 bg-white/2 border border-white/5 rounded-[2.5rem] space-y-8">
                <div className="flex items-center gap-4 text-purple-400 border-b border-white/5 pb-6">
                   <div className="w-10 h-10 bg-purple-500/10 rounded-xl flex items-center justify-center">
                     {getIcon(selectedGame.icon, "w-5 h-5")}
                   </div>
                   <h4 className="text-lg font-black uppercase tracking-tight text-white">{selectedGame.name} Portal</h4>
                </div>
                {selectedGame.requirements.map(req => (
                  <div key={req}>
                    ...
                    <input 
                      ...
                      className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-5 text-sm font-mono focus:border-purple-500/50 outline-none transition-all placeholder:text-slate-800 text-white"
                    />
                  </div>
                ))}
              </div>
          ...
          {step === 3 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="bg-purple-500/5 border border-purple-500/20 rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center gap-10">
                <div className="bg-white p-3 rounded-2xl shadow-xl shrink-0">
                  <img 
                    ...
                    className="w-32 h-32"
                  />
                  <div className="mt-2 text-center text-[8px] font-black text-slate-900 bg-purple-400 rounded-full py-0.5">ESEWA_NODE_GAMING</div>
                </div>
                <div className="flex-1 text-center md:text-left space-y-4">
                  <div>
                    <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Gateway Hub: Nipesh Basnet</p>
                    <p className="text-2xl font-mono font-bold text-purple-400 tracking-tighter">9707340249</p>
                  </div>
                  <p className="text-[11px] text-slate-400 leading-relaxed uppercase tracking-wide">
                    Finalize payment using the QR. Then upload the <span className="text-white font-black">Transaction Success</span> screenshot.
                  </p>
                </div>
              </div>

              <div className={`p-8 rounded-[2rem] border transition-all ${voucher ? 'bg-purple-500/10 border-purple-500/30' : 'bg-white/2 border-white/10'}`}>
                <div className="flex items-center justify-between gap-6">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <p className="text-xs font-black uppercase text-white tracking-widest">Success Proof</p>
                  ...
                  <button 
                    ...
                    className={`px-8 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 ${
                       voucher ? 'bg-white/5 text-slate-500' : 'bg-purple-600 text-white shadow-xl shadow-purple-600/20'
                    }`}
                  >
                    {voucher ? 'Replace' : 'Upload Receipt'}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="p-10 border-t border-white/5 bg-white/2 flex gap-4">
           ...
           <button 
            disabled={isSubmitting || (step === 1 && !selectedGame) || (step === 2 && !isFormComplete) || (step === 3 && !voucher)}
            onClick={() => {
            ...
            className={`flex-[2] py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.4em] flex items-center justify-center gap-3 transition-all active:scale-95 shadow-2xl ${
              step === 3 ? 'bg-purple-600 shadow-purple-500/30 text-white' : 'bg-white text-black'
            } disabled:opacity-30`}
           >
             {isSubmitting ? (
               <><Loader2 className="w-4 h-4 animate-spin" /> Confirming Uplink...</>
             ) : (
               <>
                 {step === 3 ? 'Finalize Credits' : 'Continue'}
